...
`⦅ c , s ⦆⇒ t` is derivable for some `c ∈ Com` and `s ∈ State` the `t ∈ State` is unique.
...
```
theorem-deterministic-bs : ∀ {c : Com} {s t t' : State} →
                          ⦅ c , s ⦆⇒ t → ⦅ c , s ⦆⇒ t' → t == t'

theorem-deterministic-bs Skip Skip = refl
theorem-deterministic-bs Loc Loc = refl
theorem-deterministic-bs (Comp hyp1 hyp3) (Comp hyp2 hyp4)
         rewrite theorem-deterministic-bs hyp1 hyp2 |
                 theorem-deterministic-bs hyp3 hyp4 = refl

theorem-deterministic-bs (IfTrue x hyp1) (IfTrue y hyp2)
         rewrite theorem-deterministic-bs hyp1 hyp2 = refl
theorem-deterministic-bs (IfTrue x hyp1) (IfFalse y hyp2)
                     = lemma-true-neq-false abs
         where
            abs : true == false
            abs = tran (symm x) y
theorem-deterministic-bs (IfFalse x hyp1) (IfTrue y hyp2)
                     = lemma-true-neq-false abs
         where
            abs : true == false
            abs = tran (symm y) x
theorem-deterministic-bs (IfFalse x hyp1) (IfFalse y hyp2)
         rewrite theorem-deterministic-bs hyp1 hyp2 = refl
theorem-deterministic-bs (WhileFalse x) (WhileFalse y) = refl
theorem-deterministic-bs (WhileFalse x) (WhileTrue y hyp2 hyp3)
                     = lemma-true-neq-false abs
         where
            abs : true == false
            abs = tran (symm y) x 
theorem-deterministic-bs (WhileTrue x hyp1 hyp3) (WhileFalse y)
                     = lemma-true-neq-false abs
         where
            abs : true == false
            abs = tran (symm x) y
theorem-deterministic-bs (WhileTrue x hyp1 hyp3) (WhileTrue y hyp2 hyp4)
         rewrite theorem-deterministic-bs hyp1 hyp2 |
                 theorem-deterministic-bs hyp3 hyp4 = refl
```
...
parts by more efficient or shorter ones. In this section we show some examples; the first
...
A slightly more complex example is the proof that whenever the brances
